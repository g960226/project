...
class Circle {
	double radius;		// 반지름
	double pi = 3.14;	// 클래스의 멤버 필드는 초기값을 미리 지정할 수 있다 // 이때는 모든 객체가 같은 값을 가질때 사용 // 객체마다 다른 값을 가질 때 - 초기값은 생성자에 만듦
	double area;		// 원의 넓이 (반지름 x 반지름 x 3.14)   cm²
	double length;		// 원의 둘레 (반지름 x 2 x 3.14)		 cm
	// 생성자 : 객체를 생성하는 메서드(method:방식)
	// 생성자 오버로딩 : 객체를 생성하는 여러 방식을 제공하겠다 // 이렇게해도 객체를 생성할 수 있고 저렇게해도 객체를 생성할 수있다
...
//		Circle(3.0);	// 생성자 내부에서 또다른 생성자를 호출할 때는 this키워드를 사용한다
		this(3.0);		// 항상 생성자 코드의 첫줄에서만 가능 !! // double이니까 double함수로 가서 값이 출력
//		radius = 3;
